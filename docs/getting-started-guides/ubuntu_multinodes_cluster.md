...
*2 All machines can communicate with each orther, but they do not have to connect to outside world (must use private docker registry in this case)*

*3 These guide is tested OK on Ubuntu 14.04 LTS 64bit server, but it should also work on most Ubuntu versions*

*4 Dependences of this guide: etcd-2.0.0, flannel-0.2.0, k8s-0.10.0, but it should also work on higher versions*


### **Main Steps**
#### I. Make *kubernetes* , *etcd* and *flanneld* binaries

> Do these on your labtop unless your nodes have access to the Internet

1. Download the latest Kubernetes releae [kubernetest binaries](https://github.com/GoogleCloudPlatform/kubernetes/blob/master/docs/getting-started-guides/binary_release.md) .

	Then copy the kuber binaries from `kubernetes/server/kubernetes/server/bin` into `/opt/bin`  of every node.

2. Similarly fetch an `etcd` and `etcdctl` binary from [etcd releases](https://github.com/coreos/etcd/releases). 

	Then copy the `etcd` and `etcdctl` binary into `/opt/bin` of every node.  

	> etcd stable version have recently upgraded from 0.4.6 to 2.0.0 This guide **supports 2.0.0 only**.
 
3. We use flanneld to build an overlay network across multiple minion machines. Follow the instructions on [coreos flanneld github main page](https://github.com/coreos/flannel) to build an executable binary. 

	Then copy the `flanneld` binary into `/opt/bin` of every node.
...
First of all, copy the `cluster/ubuntu-cluster` dirctory to **every node**，and run `cluster/ubuntu-cluster/configue.sh` interactively  **on every node**.
